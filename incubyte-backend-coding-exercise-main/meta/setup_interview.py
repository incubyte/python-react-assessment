...
def setup_interview(candidate_username: str, github_token: str):
    repo_name = "kyruus-interview-" + re.sub(
        r"[^a-z0-9]", "-", candidate_username.lower()
    )
    print("Creating repo...")
    ...
    if not repo.heads:
        ...
        for path in Path(".").iterdir():
            if (
                ...
                or (path.name.startswith(".") and path.name != ".gitignore")
                or path.name.startswith("kyruus-interview")  # for repeated runs
            ):
                continue
            print(path.absolute())
            if path.is_dir():
                shutil.copytree(
                    path.absolute(), f"{repo_name}/{path.name}", dirs_exist_ok=True
                )
            else:
